...
class Coleccion():
    ...
    #--------------------------------------------------------#
    #Métodos de Interface IDirector
    def modificarBasico(self,dni,nuevoBasico):
        print("Modificación de Sueldo Básico")
        bandera= False
        while not bandera and self.__index < self.__tope:
            nodo= self.__next__()
            agente= nodo.getDato()
    ...
    def modificarPorcentajeporcargo(self,dni,nuevoPorcentaje):
        print("Modificación de porcentaje por cargo")
        bandera= False
        while not bandera and self.__index < self.__tope:
            nodo= self.__next__()
            agente= nodo.getDato()
            if agente.getCuil().split('-')[1] == dni:
                bandera= True
                cargo= input("Ingrese el cargo que desea modificar: ")
                agente.modificarPorcentajeCargo(cargo,nuevoPorcentaje)   
                print("Operación exitosa!")
        self.__actual= self.__comienzo
        
    def modificarPorcentajeporcategoria(self,dni,nuevoPorcentaje):
        print("Modificación de porcentaje por categoria")
        bandera= False
        while not bandera and self.__index < self.__tope:
            nodo= self.__next__()
            agente= nodo.getDato()
            if agente.getCuil().split('-')[1] == dni:
                bandera= True
                categoria= input("Ingrese la categoria que desea modificar: ")
                agente.modificarPorcentajeCategoria(categoria,nuevoPorcentaje)   
                print("Operación exitosa!")
        self.__actual= self.__comienzo
